...
class BurstSRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("1", torch.min(gt_image_data), torch.max(gt_image_data))
        ...
        # print("exp_scale_factor",exp_scale_factor)
        # print("2", torch.min(gt_image_data), torch.max(gt_image_data))
        ...
        # print("3", torch.min(gt_image_data), torch.max(gt_image_data))
        ...
        meta_info_gt['burst_name'] = burst_name

        for k, v in meta_info_burst.items():
            if isinstance(v, (list, tuple)):
                meta_info_burst[k] = torch.tensor(v)
        meta_info_burst['burst_name'] = burst_name

        if self.split == "train":
            frame_gt = frame_gt.clamp(0.0,1.0)
            # bgr -> rgb
            frame_gt = frame_gt * 2 - 1
            frame_gt_rgb = frame_gt.clone().detach()
            frame_gt_rgb[0], frame_gt_rgb[2] = frame_gt[2], frame_gt[0]
            frame_gt = frame_gt_rgb.clone().detach()
