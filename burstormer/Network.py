...
class no_ref_back_projection_from_denoise(pl.LightningModule):
    def __init__(self, in_channels, stride):

        super(no_ref_back_projection_from_denoise, self).__init__()

        bias = False
        
        self.feat_fusion = nn.Conv2d(in_channels*2, in_channels, 3, stride=1, padding=1, bias=bias)        
        self.feat_expand = nn.Conv2d(in_channels, in_channels*2, 3, stride=1, padding=1, bias=bias)
        self.diff_fusion = nn.Conv2d(in_channels*2, in_channels, 3, stride=1, padding=1, bias=bias)
    ...
    def forward(self, x):
        ...
        fused_feat = self.act(self.feat_fusion(feat))
        rec_feat = self.act(self.feat_expand(fused_feat))

        residual = self.act(self.diff_fusion(feat - rec_feat))
        fused_feat = fused_feat + residual
...
class burstormer(pl.LightningModule):
    def __init__(self, num_features=48, burst_size=8, reduction=8, bias=False):
        ...
        self.back_projection1 = no_ref_back_projection_from_denoise(num_features, stride=1)
        self.back_projection2 = no_ref_back_projection_from_denoise(num_features, stride=1)
    ...
    def forward(self, burst):  #burst.shape=[1, 14, 4, 48, 48]
        #特徴量生成過程
        burst = burst[0]
